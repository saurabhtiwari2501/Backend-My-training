...
const createBook = async function (req, res) {
    try {
        ...
        if (Object.keys(data).length != 0) {
            ...
            res.status(201).send({ msg: savedData }) // 201 means user crested
        }
...
const updateBooks = async function (req, res) {
    ...
    if(bookModel.findOneAndUpdate) res.status(202).send({ msg: allBooks })
    else res.status(405).send({msg: "Not Allowed", condition: false})
}
